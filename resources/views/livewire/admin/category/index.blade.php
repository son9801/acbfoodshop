<div>
    <div wire:ignore.self class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Xoá danh mục</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form wire:submit.prevent="destroyCategory">
                    <div class="modal-body">
                        <h6>Bạn có chắc chắn muốn xoá danh mục này không? </h6>
                        <h6 style="color:red">LƯU Ý: Các sản phẩm thuộc danh mục này cũng sẽ bị xoá</h6>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
                        <button type="Submit" data-bs-dismiss="modal" class="btn btn-primary text-white">Đồng ý xoá </button>
                    </div>
                </form>
            </div>
        </div>
    </div>


    <div class="row">

        <div class="col-md-12">

            @if (session('message'))
                <div class="alert alert-success">{{ session('message') }}</div>
            @endif

            <div class="card">
                <div class="card-header">
                    <h4>Danh mục
                        <a href="{{ url('admin/category/create') }}" class="btn btn-primary btn-sm float-end text-white">Thêm danh
                            mục</a>
                    </h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <label for="">Tìm kiếm theo tên</label>
                                <input type="search" type="text" class="form-control" wire:model="searchKey"
                                    placeholder="Nhập tên danh mục muốn tìm">
                            </div>
                        </div>
                    </div>
                    <table class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Tên</th>
                                <th>Ảnh</th>
                                <th>Trạng thái</th>
                                <th>Quản lý</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if ($categories)
                                @foreach ($categories as $category)
                                    <tr>
                                        <td>{{ $category->id }}</td>
                                        <td>{{ $category->name }}</td>
                                        <td><img src="{{ asset($category->image) }}"
                                                style="width:100px;height:100px;border-radius:0%" alt="Slider"></td>
                                        <td>{{ $category->status == '1' ? 'Hiện' : 'Ẩn' }}</td>
                                        <td>
                                            <a href="{{ url('admin/category/' . $category->id . '/edit') }}"
                                                class="btn btn-success">Sửa</a>
                                            <a href="#" wire:click="deleteCategory({{ $category->id }})"
                                                data-bs-toggle="modal" data-bs-target="#deleteModal"
                                                class="btn btn-danger">Xoá</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
                    </table>
                    <div>
                        {{ $categories->links() }}
                    </div>
                </div>
            </div>
        </div>
    </div>
